...
class tank:
    ...
    def draw(self):
        ...
        if self.team == 1:
            ...
            pygame.draw.circle(screen, (0, 0, 255), (self.X[0], self.X[1]), 2)
    ...
    def lidarSingle(self, d):
        ...
        if _tmin is None or _bmin is None:
            ...
            if _bmin is None:
                return [_tmin, tank.tanks[_tmini].td - d, tank.tanks[_tmini].d - d,
                        tank.tanks[_tmini].team] + _lidarWalls + [10]
            if _tmin is None:
                return _lidarWalls + [10, 10, 10] + [_bmin, bullet.bullets[_bmini].d - d]
        return [_tmin, tank.tanks[_tmini].td - d, tank.tanks[_tmini].d - d, tank.tanks[_tmini].team] + [_bmin,
                                                                                                        bullet.bullets[
                                                                                                            _bmini].d - d]
    ...
    def clone(self):
        ...
        # print(_newLidarSize)
        tank([_randomx, _randomy], _randomd, _randomd, _newLidarSize, self.nLidar, self.nTurretLidar, _newModel,
             self.nRecurrence, _newRate)
    ...
    def clone2(self, otherTank):
        if random(0, 1) < pRandomTank:
            ...
            tank([_randomX, _randomY], _randomD, 10, 10, nLidarTurret, nLidarBody, _initialModel, nRecurrence,
                 initialRate)
            return 0
        ...
        if len(tank.tanks) < numberofTanks + 2:
            ...
            _newTank = tank([_randomx, _randomy], _randomd, _randomd, 10, self.nLidar, self.nTurretLidar, self.model,
                            self.nRecurrence, initialRate)
    ...
    def collisionDetection(self):
        for i in range(len(tank.tanks)):
            if tank.tanks[i] is not self.tank and isL1Collisoin(self.X, tank.tanks[i].X,
                                                                (self.size[0] + tank.tanks[i].size[0]) / 2) and \
                    tank.tanks[i].alive == 1:
                self.deletion = 1
...
def loadModels(directory):
    ...
    for i in range(len(data[0])):
        ...
        tank([_randomX, _randomY], 0, 0, data[0][i][2], nLidarBody, nLidarTurret, data[0][i][0], nRecurrence,
             data[0][i][1])
...
def round(m0, m1, n, generationModels):
    _winner = None

    for i in range(len(tank.tanks)):
        tank.tanks[i].deletion = 1

    while (len(tank.tanks) != 0):
        tank.tanks[0].delete()

    tank.teamN[0] = copy.deepcopy(n)
    tank.teamN[1] = copy.deepcopy(n)

    for i in range(n):
        tank([0, random(0, window_height)], random(0, 2 * np.pi), random(0, 2 * np.pi), lidarSize, nLidarBody,
             nLidarTurret, m0, nRecurrence, initialRate, 0)
        tank([window_width, random(0, window_height)], random(0, 2 * np.pi), random(0, 2 * np.pi), lidarSize,
             nLidarBody, nLidarTurret, m1,
             nRecurrence, initialRate, 1)
    ...
    while _roundRunning:
        ...
        if _updatePosTimer < 1:
            ...
            if _updatePosCount == staleReset:
                return [m0, tank.teamN[0]-tank.teamN[1]]
        ...
        screen.blit(font.render("Team0:" + str(tank.teamN[0]), 1, pygame.Color("coral")), (10, 20))
        screen.blit(font.render("Team1:" + str(tank.teamN[1]), 1, pygame.Color("coral")), (10, 40))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                _roundRunning = False
                pickle.dump(generationModels, open(saveTo, "wb"))

        if tank.teamN[0] == 0 and tank.teamN[1] == 0:
            #print("rematch")
            return round(m0, m1, n)
        elif tank.teamN[0] == 0:
            #print("m1 wins")
            return [m1, tank.teamN[1]]
        elif tank.teamN[1] == 0:
            #print("m2 wins")
            return [m0, tank.teamN[0]]


def evalGeneration(m, n, generationModels):
    _round = 0
    ...
    for i in range(len(models) - 1):
        for j in range(len(models) - i - 1):
            ...
            k = i + j + 1
            print(i,"vs",k)
            result = round(models[i][0], models[k][0], n, generationModels)

            if result[0] == models[i][0]:
                models[i][1] += result[1]
                models[k][1] -= result[1]
            else:
                models[k][1] += result[1]
...
def cloneGeneration(m):
    models = copy.deepcopy(m)
    groups = []
    while (len(models) > 2):
        groups.append([models[0], models[1], models[2]])
        for i in range(3):
            models.pop(0)

    if len(models) == 2:
        groups.append([models[0], models[1]])
        for i in range(2):
            models.pop(0)
    if len(models) == 1:
        groups.append([models[0]])
        models.pop(0)
    #print(groups)
    #print(len(groups))
    for i in range(len(groups)):
        #print(i)
        if len(groups[i]) == 3:
            _worstModel = groups[i][0]
            _worstModeli = 0

            for j in range(2):
                if groups[i][j + 1][1] < _worstModel[1]:
                    _worstModel = groups[i][j + 1]
                    _worstModeli = j + 1

            groups[i].pop(_worstModeli)

            p1 = groups[i][0][0]
            p2 = groups[i][1][0]

            _newModel = Network([(nLidarBody + nLidarTurret) * 6 + nRecurrence + 4] + networkShape + [7 + nRecurrence],
                                Activation_ReLU)
            ...
            for i_ in range(_newModel.nlayers):
                for j in range(len(_newModel.layers[i_].weights)):
                    for k in range(len(_newModel.layers[i_].weights[j])):
                        if np.random.rand() > 0.5:
                            _newModel.layers[i_].weights[j][k] = copy.deepcopy(p1.layers[i_].weights[j][k])
                        else:
                            _newModel.layers[i_].weights[j][k] = copy.deepcopy(p2.layers[i_].weights[j][k])
                for j in range(len(_newModel.layers[i_].biases)):
                    for k in range(len(_newModel.layers[i_].biases[j])):
                        if np.random.rand() > 0.5:
                            _newModel.layers[i_].biases[j][k] = copy.deepcopy(p1.layers[i_].biases[j][k])
                        else:
                            _newModel.layers[i_].biases[j][k] = copy.deepcopy(p2.layers[i_].biases[j][k])

            _newModel.child(initialRate)

            #print(groups,i)
            groups[i].append([_newModel, 0])

    for i in range(len(groups)):
        for j in range(len(groups[i])):
            models.append(groups[i][j])

    _newModels = []

    while len(models) != 0:
        _random = np.random.randint(0,len(models))
        _newModels.append(models[_random])
        models.pop(_random)
...
if loadFrom == 0:
    models = []
    for i in range(6):
        models.append([Network([(nLidarBody + nLidarTurret) * 6 + nRecurrence + 4] + networkShape + [7 + nRecurrence],
                               Activation_ReLU), 0])
else:
...
for i in range(1000):
    print("generation =", i)
    models = cloneGeneration(evalGeneration(models, 4, models))

pickle.dump(models, open(saveTo, "wb"))

# print((nLidarBody + nLidarTurret) * 6 + nRecurrence + 3)

# m0 = Network([(nLidarBody + nLidarTurret) * 6 + nRecurrence + 3] + networkShape + [7 + nRecurrence],
#                                Activation_ReLU)
# m1 = Network([(nLidarBody + nLidarTurret) * 6 + nRecurrence + 3] + networkShape + [7 + nRecurrence],
#                                Activation_ReLU)
